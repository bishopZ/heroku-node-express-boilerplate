...
(function(global){
	...
	Frame.clear = function(){ return (_queue = []) && (_qArgs = []); } // clear the queue
	Frame.clearAll = function(){ return (_queue = []) && (_qArgs = []) && (_later = []) && (_lArgs = []); } // clear soon & later queue
		...
		if (typeof fn === 'function') { 
			...
			try { // only catches some errors, but has minimal speed impact and adds safety
				...
				// callback is next() instead of Frame(), so waterfall args can be passed in
	 			...
	 			_clear();
	 			Frame.error(e, optional(e.arguments), Frame.last);
	 			Frame(); // move on
	 			return e;
	 		}
	 		return true;
